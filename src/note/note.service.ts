...
export class NoteService {
  ...
  async createNote(data: Prisma.NoteCreateInput): Promise<Note> {
    return this.prisma.note.create({ data, include: { user: true } });
  }
 ...
 //devuelve notas que contenga las palabras pasadas en el title o description
  async searchNotes(search: string): Promise<Note[]> {
    return this.prisma.note.findMany({
      where: {
        OR: [
